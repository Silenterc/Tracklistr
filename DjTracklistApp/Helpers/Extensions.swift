...
extension Int {
    ...
    func getBars(bpm: Float, timeUnit: TimeUnit, timeSignature: Int = 4) -> Int {
        // Bars get calculated with the formula: Bars = (AmountOfMinutes * Beats Per Minute)/Time Signature
        switch (timeUnit) {
        case .milliseconds: return Int((Double(self) * Double(bpm)) / (Double(timeSignature) * 60000.0))
        case .seconds: return Int((Double(self) * Double(bpm)) / (Double(timeSignature) * 60.0))
        case .minutes: return Int((Double(self) * Double(bpm)) / Double(timeSignature))
        }
